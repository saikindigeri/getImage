  ...
  return (
    <section id="testimonials" className="py-12 mb-20 px-4 sm:px-6 ">
      <div className="max-w-6xl mx-auto relative">
        ...
        <motion.button
          onClick={handlePrev}
          className="absolute left-[-48px] sm:left-[-60px] top-1/2 transform -translate-y-1/2 p-3"
        ...
        <motion.button
          onClick={handleNext}
          className="absolute right-[-48px] sm:right-[-60px] top-1/2 transform -translate-y-1/2 p-3"
          whileHover={{ scale: 1.1 }}
          whileTap={{ scale: 0.95 }}
          transition={{ duration: 0.2 }}
        ...
        <div className="flex justify-center">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="w-full max-w-3xl p-6  rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 flex flex-col items-center text-center"
            >
              {/* User Image */}
              ...
              <p className={`${inter} text-sm text-gray-500 dark:text-gray-400 mb-3`}>
                {currentTestimonial.role}
              </p>

              {/* Feedback */}
              <p className={`${inter} text-sm text-gray-600 dark:text-gray-300 italic`}>
              &quot {currentTestimonial.feedback} &quot
              </p>
            </motion.div>
          </AnimatePresence>
        </div>
